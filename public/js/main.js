...
$(document).ready(function(){
	$('#KiemTraTaoTruyen').submit(function(){
        ...
        else if ($('#DeCu').is(":checked")){
        	if($('#AnhBia').val() == ''){
        		Alert('<b>BẠN PHẢI CHỌN ẢNH BÌA MỚI CÓ THỂ ĐỀ CỬ TRUYỆN NÀY!</b>');
				event.preventDefault();
        	}	
		}
...
function FeedBack(){
	$.confirm({
	    ...
	    '<textarea class="form-control name" rows="3" placeholder="Tên lỗi..." maxlength="500"></textarea>' +
	    '</div>' +
	    '</form>',
	    type: 'orange',
	    buttons: {
	        formSubmit: {
	            ...
	            action: function () {
	                ...
	                $.post("index.php?url=ajax-them-feedback", {id: idTruyen, content: name}, function(data){
						if(data == 1){
							Alert('Phản hồi của bạn đã được ghi nhận, chúng tôi sẽ nhanh chóng khắc phục!');
						}
						else{
							Alert(data);
						}
